...
class CSNN(nn.Module):
    def __init__(self, T, channels, classes, use_cupy=False):
        super().__init__()
        self.T = T
...
def main():
    ...
    # 初始化模型
    net = CSNN(T=args.T, channels=args.channels, classes=args.classes, use_cupy=args.cupy)
    print(net)
    net.to(args.device)
    ...
    # 设置输出目录
    out_dir = os.path.join(args.out_dir, f'T{args.T}_b{args.b}_{args.opt}_lr{args.lr}_c{args.channels}_tau∞')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if save_max:
            ...
            # 保存最佳模型的参数
            save_model_parameters(net, f'../../../../save_model_parameter/MNIST/1Conv2Linear/T{args.T}', '∞')
    ...
    plot_metrics(train_accuracies, test_accuracies, 'Accuracy', acc_save_dir, args.dataset_name, f'tau(∞)')

    # 绘制并保存损失曲线
    loss_save_dir = os.path.join('../../../../save_img/MNIST/1Conv2Linear', f'loss/T{args.T}')
    plot_metrics(train_losses, test_losses, 'Loss', loss_save_dir, args.dataset_name, f'tau(∞)')
